    ...
    : QMainWindow(parent)
{
    ...
    // to separate the control panel fromthe image viewing area.
    // ==========================================
...
QGroupBox("Parameters");
    ...
    // Connect signals to slots for corefunctionality
...
int MainWindow::otsuThresholdScratch(const cv::Mat& gray) {
    ...
    // 2. Iterate through all possible thresholds to maximize variance
    for (int i = 0; i < 256; i++) {
...
MainWindow::spectralThresholdScratch(const cv::Mat& gray, int numModes) {
    ...
    for(int m = 0; m < numModes; m++) {
        ...
        if(bestPeak != -1 && maxVal > 0) {
            ...
            // Erase a neighborhood around the detected peak to find the nextdistinct mode
            int radius = 20;
...
cv::Mat MainWindow::kmeansScratch(const cv::Mat& img, int k, int maxIter) {
    ...
    // Step 1: Initialize cluster centersrandomly
